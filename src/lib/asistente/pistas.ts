...
export const PISTAS: Pista[] = [
  ...
  { id: "clientas-inactivas", re: r(`^(?!.*\\b(?:nuev[oa]s?|primera vez|una (?:sola )?vez)\\b).*(?:\\bno (?:vuelven|vuelve|han vuelto)\\b|\\bsin venir\\b|\\bhace (?:mucho|tiempo) que no viene\\b|\\bperdidas\\b|\\blleva(?:n)? tiempo sin\\b|\\binactiv\\w*|\\bsin pisar\\b|\\bparadas\\b|\\bno (?:ha|han) venido desde hace\\b|\\bhace \\d+ (?:meses|semanas) que no\\b)`), peso: 0.5 },
  ...
  { id: "cobro-por-metodo", re: r(`\\b(?:tarjeta|efectivo|forma de pago|metodo de pago|desglose)\\b|\\bcobr\\w* (?:por|en) bizum\\b`), peso: 0.4 },
  ...
  { id: "senales-vencidas", re: r(`\\b(?:vencid\\w*|caducad\\w*|fuera de plazo|pasado el plazo|pasaron de plazo|se (?:le |les )?(?:ha )?pas(?:o|aron|ado) (?:el|de) plazo)\\b`), peso: 0.45 },
  ...
  { id: "calendario-suscrito", re: r(`\\bgoogle calendar\\b|\\b(?:calendario|agenda|citas)\\b.*\\b(?:iphone|android|movil)\\b|\\b(?:iphone|android|movil)\\b.*\\b(?:calendario|agenda|citas)\\b|\\bcalendario (?:de google|del movil)\\b|\\bsincroniz\\w*\\b(?!.*(?:dos sentidos|tpv))`), peso: 0.4 },
  ...
  { id: "regla-senal", re: r(`^(?!.*\\b(?:pasad\\w*|paso|pasaron|vencid\\w*|fuera de|caducad\\w*)\\b).*\\bplazo\\b`), peso: 0.45 },
  { id: "estimacion-mes", re: r(`\\bvamos a (?:terminar|acabar|cerrar)\\b|\\bcomo (?:terminaremos|acabaremos)\\b`), peso: 0.5 },
  { id: "huecos-flojos", re: r(`\\bhueco flojo\\b|\\bllenarme\\b`), peso: 0.5 },
  { id: "solicitudes-pendientes", re: r(`\\besperando confirmaci\\w*|\\bpendientes? de (?:confirmar|aceptar)\\b`), peso: 0.5 },
  { id: "tec-no-llegan-reservas", re: r(`\\bsin (?:solicitudes|reservas)(?: nuevas)?\\b.*\\b(?:dias|semana|pasar|pasa)\\b|\\bque (?:puede|le) pasa\\w*\\b.*\\breserv`), peso: 0.5 },
  { id: "no-campanas-automaticas", re: r(`\\bsms\\b|\\bmandar a todas\\b|\\ba todas mis clientas\\b`), peso: 0.55 },
  { id: "que-sabes-hacer", re: r(`\\bno se (?:ni )?que preguntar\\w*|\\bno se que (?:te )?puedo preguntar\\b`), peso: 0.8 },
