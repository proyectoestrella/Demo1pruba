...
export type ResultadoAccesos = { ok: true } | { ok: false; codigo: "PLAN" | "ULTIMA_GERENTE" | "VINCULO" | "DUPLICADO"; mensaje: string };
...
const correoDe = (nombre: string) =>
  `${nombre.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-z]+/g, ".")}@ejemplo.com`;
...
export function miembrosDeDemo(equipo: Employee[]): Miembro[] {
  const lista: Miembro[] = equipo.map((e, i) => {
    ...
    return { userId: `demo-${e.id}`, rol: i === 0 ? "gerente" : "estilista", employeeId: e.id, displayName: nombre, email: correoDe(nombre), estado: "activa" };
  });
  lista.push({ userId: "demo-subencargada", rol: "subencargado", employeeId: null, displayName: "Laura", email: "laura@ejemplo.com", estado: "activa" });
  return lista;
}
...
const gerentesActivas = (m: Miembro[]) => m.filter((x) => x.rol === "gerente" && x.estado === "activa");
...
export const accesos = {
  ...
  invitarMiembro(d: { email: string; rol: Rol; employeeId: string | null; displayName: string | null }, plan: string | null, ahora = new Date()): ResultadoAccesos {
    const m = useAccesosDemo.getState().miembros ?? [];
    const email = d.email.trim().toLowerCase();
    if (activos(m).some((x) => x.email.toLowerCase() === email)) return { ok: false, codigo: "DUPLICADO", mensaje: "Ese correo ya tiene acceso o una invitación." };
    if (d.employeeId && activos(m).some((x) => x.employeeId === d.employeeId)) return { ok: false, codigo: "VINCULO", mensaje: "Esa profesional ya tiene su cuenta." };
    if (d.rol === "estilista" && !d.employeeId) return { ok: false, codigo: "VINCULO", mensaje: "Elige quién es en tu equipo." };
    if (!cabeRol(d.rol, activos(m).map((x) => x.rol), plan)) return { ok: false, codigo: "PLAN", mensaje: "Tu plan incluye dos tipos de acceso (gerente y estilista)." };
    const caduca = new Date(ahora);
    caduca.setDate(caduca.getDate() + DIAS_INVITACION);
    useAccesosDemo.getState().aplicar((l) => [
      ...l,
      { userId: `inv-${Date.now()}`, rol: d.rol, employeeId: d.employeeId, displayName: d.displayName, email, estado: "invitada", invitadaEn: ahora.toISOString(), caducaEn: caduca.toISOString() },
    ]);
    return { ok: true };
  },
  ...
  reenviarInvitacion(userId: string, ahora = new Date()): ResultadoAccesos {
    const caduca = new Date(ahora);
    caduca.setDate(caduca.getDate() + DIAS_INVITACION);
    useAccesosDemo.getState().aplicar((l) => l.map((x) => (x.userId === userId ? { ...x, invitadaEn: ahora.toISOString(), caducaEn: caduca.toISOString() } : x)));
    return { ok: true };
  },
  ...
  cambiarRol(userId: string, rol: Rol, plan: string | null): ResultadoAccesos {
    ...
    if (yo.rol === "gerente" && rol !== "gerente" && gerentesActivas(m).length <= 1)
      return { ok: false, codigo: "ULTIMA_GERENTE", mensaje: "Tiene que quedar al menos una gerente." };
    if (rol === "estilista" && !yo.employeeId) return { ok: false, codigo: "VINCULO", mensaje: "Para ser estilista, vincúlala antes a una profesional del equipo." };
    if (!cabeRol(rol, m.filter((x) => x.userId !== userId).map((x) => x.rol), plan))
      return { ok: false, codigo: "PLAN", mensaje: "Tu plan incluye dos tipos de acceso (gerente y estilista)." };
    useAccesosDemo.getState().aplicar((l) => l.map((x) => (x.userId === userId ? { ...x, rol } : x)));
    return { ok: true };
  },

  vincular(userId: string, employeeId: string | null): ResultadoAccesos {
    const m = activos(useAccesosDemo.getState().miembros ?? []);
    if (employeeId && m.some((x) => x.userId !== userId && x.employeeId === employeeId)) return { ok: false, codigo: "VINCULO", mensaje: "Esa profesional ya tiene su cuenta." };
    const yo = m.find((x) => x.userId === userId);
    if (yo?.rol === "estilista" && !employeeId) return { ok: false, codigo: "VINCULO", mensaje: "Una estilista tiene que estar vinculada a una profesional." };
    useAccesosDemo.getState().aplicar((l) => l.map((x) => (x.userId === userId ? { ...x, employeeId } : x)));
    return { ok: true };
  },
  ...
  darDeBaja(userId: string): ResultadoAccesos {
    const m = activos(useAccesosDemo.getState().miembros ?? []);
    const yo = m.find((x) => x.userId === userId);
    if (yo?.rol === "gerente" && gerentesActivas(m).length <= 1) return { ok: false, codigo: "ULTIMA_GERENTE", mensaje: "Tiene que quedar al menos una gerente." };
    useAccesosDemo.getState().aplicar((l) => l.map((x) => (x.userId === userId ? { ...x, estado: "baja" } : x)));
  ...
  restaurar(miembro: Miembro): void {
    useAccesosDemo.getState().aplicar((l) => (l.some((x) => x.userId === miembro.userId) ? l.map((x) => (x.userId === miembro.userId ? miembro : x)) : [...l, miembro]));
  },
};
