...
export type ResultadoAccesos<T = undefined> =
  ...
  | { ok: false; codigo: "PERMISO" | "PLAN" | "DATOS" | "ULTIMA_GERENTE" | "NO_EXISTE" | "CADUCADA" | "OTRO_CORREO" | "YA_MIEMBRO"; motivo: string };
...
function rolesEnUso(m: MiembroFila[], inv: InvitacionFila[]): Rol[] {
  return [...m.filter((x) => x.estado === "activa").map((x) => x.rol), ...inv.filter((i) => !i.aceptadaEn && !i.revocada).map((i) => i.rol)];
}
...
export async function invitarMiembro(
  ...
  if (!correoValido(email)) return { ok: false, codigo: "DATOS", motivo: "Ese correo no parece válido." };
  if (!esRol(datos.rol)) return { ok: false, codigo: "DATOS", motivo: "Ese rol no existe." };
  const rol = datos.rol;
  if (rol === "estilista" && !datos.employeeId) return { ok: false, codigo: "DATOS", motivo: "Elige qué profesional es: una estilista ve su agenda." };
  const [miembros, invs, plan] = await Promise.all([deps.miembros(), deps.invitaciones(), deps.plan()]);
  if (miembros.some((m) => m.estado === "activa" && m.email && norm(m.email) === email)) return { ok: false, codigo: "YA_MIEMBRO", motivo: "Esa persona ya tiene acceso." };
  if (rol === "estilista" && miembros.some((m) => m.estado === "activa" && m.employeeId === datos.employeeId))
  ...
  if (!cabeRol(rol, rolesEnUso(miembros, invs), plan))
    return { ok: false, codigo: "PLAN", motivo: "Tu plan permite dos tipos de acceso. Para más, el plan Todo incluido." };
...
export async function aceptarInvitacion(userId: string, invitacionId: string, deps: DepsAccesos): Promise<ResultadoAccesos> {
  const inv = (await deps.invitaciones()).find((i) => i.id === invitacionId);
  if (!inv || inv.revocada) return { ok: false, codigo: "NO_EXISTE", motivo: "Esta invitación ya no es válida. Pide otra a quien gestiona el salón." };
  if (inv.aceptadaEn) return { ok: true };
  if (deps.ahora().getTime() > Date.parse(inv.caduca)) return { ok: false, codigo: "CADUCADA", motivo: "La invitación ha caducado. Pide que te la reenvíen." };
  const correo = await deps.correoDe(userId);
  if (!correo || norm(correo) !== inv.email) return { ok: false, codigo: "OTRO_CORREO", motivo: "Has entrado con otro correo distinto del invitado." };
  await deps.guardarMiembro({ userId, email: inv.email, rol: inv.rol, employeeId: inv.employeeId, displayName: inv.displayName, estado: "activa" });
...
export async function cambiarRol(
  ...
  if (m.rol === "gerente" && nuevo.rol !== "gerente" && (await gerentesActivas(deps)).length <= 1)
    return { ok: false, codigo: "ULTIMA_GERENTE", motivo: "Tiene que quedar al menos una gerente." };
  if (nuevo.rol === "estilista" && !nuevo.employeeId) return { ok: false, codigo: "DATOS", motivo: "Elige qué profesional es." };
  ...
  if (!cabeRol(nuevo.rol, rolesEnUso(otros, await deps.invitaciones()), await deps.plan()))
    return { ok: false, codigo: "PLAN", motivo: "Tu plan permite dos tipos de acceso. Para más, el plan Todo incluido." };
  await deps.guardarMiembro({ ...m, rol: nuevo.rol, employeeId: nuevo.rol === "estilista" ? nuevo.employeeId ?? null : null });
  return { ok: true };
}

export async function darDeBaja(acceso: Acceso, userId: string, deps: DepsAccesos): Promise<ResultadoAccesos> {
  exigirGestion(acceso);
...
export async function revocarInvitacion(acceso: Acceso, invitacionId: string, deps: DepsAccesos): Promise<ResultadoAccesos> {
  exigirGestion(acceso);
  const inv = (await deps.invitaciones()).find((i) => i.id === invitacionId);
  if (!inv || inv.aceptadaEn) return { ok: false, codigo: "NO_EXISTE", motivo: "No hay invitación pendiente." };
  await deps.guardarInvitacion({ ...inv, revocada: true });
  return { ok: true };
}

export async function reenviarInvitacion(acceso: Acceso, invitacionId: string, deps: DepsAccesos): Promise<ResultadoAccesos> {
  exigirGestion(acceso);
  const inv = (await deps.invitaciones()).find((i) => i.id === invitacionId);
  if (!inv || inv.aceptadaEn || inv.revocada) return { ok: false, codigo: "NO_EXISTE", motivo: "No hay invitación pendiente." };
  const caduca = new Date(deps.ahora().getTime() + DIAS_CADUCIDAD_INVITACION * 86_400_000).toISOString();
...
export async function listarAccesos(acceso: Acceso, deps: DepsAccesos) {
  ...
  return {
    ...
    invitaciones: i.filter((x) => !x.aceptadaEn && !x.revocada).map((x) => ({ ...x, caducada: t > Date.parse(x.caduca) })),
  };
}
