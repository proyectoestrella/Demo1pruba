...
export function useMiembroActual(): Miembro | null {
  ...
  const demo = esDemo ? (activos.find((m) => m.userId === verComo) ?? activos.find((m) => m.rol === "gerente") ?? null) : null;
  ...
  useEffect(() => {
    ...
    if (demo ? actual?.userId !== demo.userId || actual?.rol !== demo.rol : actual !== null) {
      st.setMiembro(demo ? { userId: demo.userId, rol: demo.rol, employeeId: demo.employeeId, displayName: demo.displayName } : null);
    }
