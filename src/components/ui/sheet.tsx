...
import { registrarPanelLateral } from "@/lib/panel-lateral";
...
/** Se monta solo con el panel abierto: así el armazón sabe que tiene que dejarle sitio. */
function RegistroPanelLateral() {
  React.useEffect(() => registrarPanelLateral(), []);
  return null;
}
  ...
  "fixed z-50 gap-4 bg-background p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
        ...
        right:
          "inset-y-0 right-0 h-full w-full border-l border-border shadow-[var(--sombra-panel)] data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-[440px] lg:shadow-none",
      },
    },
    defaultVariants: {
      side: "right",
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      {side === "right" && <RegistroPanelLateral />}
