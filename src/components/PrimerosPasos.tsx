...
export function PrimerosPasos({ enAjustes = false }: { enAjustes?: boolean }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-primary/20 bg-card shadow-sm" aria-label="Primeros pasos">
      <button type="button" className="flex min-h-16 w-full items-center gap-3 p-4 text-left sm:px-5" aria-expanded={abierto} onClick={() => setAbierto(!abierto)}>
        <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary"><ListChecks className="size-5" /></span>
        <span className="min-w-0 flex-1"><span className="block font-display text-lg">Primeros pasos</span><span className="block text-sm text-muted-foreground">{progreso} de 6 listos</span></span>
      ...
      {abierto && <div className="border-t border-border/60 p-3 sm:p-5">
        <ol className="space-y-2">
          {PASOS.map((paso, i) => {
            const done = marcados.has(i);
            const automatico = automáticos.has(i);
            return <li key={paso.titulo} className="flex min-h-14 items-center gap-3 rounded-xl px-2 py-2 hover:bg-muted/50">
              <button type="button" disabled={automatico} onClick={() => marcar(i, !done)} aria-label={`${done ? "Desmarcar" : "Marcar"}: ${paso.titulo}`} aria-pressed={done} className={cn("grid size-7 shrink-0 place-items-center rounded-full border-2", done ? "border-primary bg-primary text-primary-foreground" : "border-muted-foreground/40 bg-background", automatico && "cursor-default")}>
              ...
              <Link to={paso.to} className="min-w-0 flex-1 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                <span className={cn("block text-sm font-medium", done && "text-muted-foreground line-through")}>{i + 1}. {paso.titulo}</span><span className="block text-xs text-muted-foreground">{paso.detalle}{automatico ? " · Listo" : ""}</span>
              </Link>
              <Link to={paso.to} aria-label={`Abrir: ${paso.titulo}`} className="rounded-md p-2 text-primary"><ExternalLink className="size-4" /></Link>
            </li>;
          })}
          <li className="flex min-h-14 items-center gap-3 rounded-xl px-2 py-2 hover:bg-muted/50">
            <button type="button" onClick={() => marcar(tareaCompartir, !marcados.has(tareaCompartir))} aria-label={`${marcados.has(tareaCompartir) ? "Desmarcar" : "Marcar"}: Compartir enlace de reservas`} aria-pressed={marcados.has(tareaCompartir)} className={cn("grid size-7 shrink-0 place-items-center rounded-full border-2", marcados.has(tareaCompartir) ? "border-primary bg-primary text-primary-foreground" : "border-muted-foreground/40 bg-background")}>{marcados.has(tareaCompartir) && <Check className="size-4" />}</button>
            <span className="min-w-0 flex-1"><span className="block text-sm font-medium">6. Comparte tu enlace de reservas</span><span className="block text-xs text-muted-foreground">En Instagram, Google o tu web</span></span>
        ...
        {!enAjustes && <div className="mt-3 flex justify-end border-t border-border/60 pt-3"><Button type="button" variant="ghost" size="sm" className="gap-2" onClick={() => update({ setupChecklistHidden: true })}><EyeOff className="size-4" />Ocultar estos pasos</Button></div>}
      </div>}
    </section>
  );
}
