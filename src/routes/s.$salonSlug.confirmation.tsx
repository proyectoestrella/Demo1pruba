...
function Confirmation() {
  ...
  if (!service || !employee) {
    return (
      <section className="mx-auto max-w-md px-5 py-24 text-center">
        ...
        <Link to="/s/$salonSlug/book" params={{ salonSlug }} className="mt-4 inline-block text-sm text-primary underline underline-offset-2">
          Volver a reservar
        </Link>
      </section>
    );
  ...
  const dateLabel = date
    ? new Date(`${date}T${time || "00:00"}`).toLocaleDateString("es-ES", { weekday: "long", day: "numeric", month: "long" })
    : "—";
    ...
    <section className="mx-auto max-w-2xl px-5 py-16 md:py-24">
      ...
      <div className="mt-10 rounded-3xl border border-border bg-card p-8">
        <div className="flex items-center justify-between">
          ...
          <span className="rounded-full bg-[var(--success)]/15 px-2.5 py-0.5 text-xs font-medium text-[var(--success)]">Confirmada</span>
        </div>
        ...
        <div className="space-y-3 text-sm">
          ...
          {requiresDeposit(service.durationMin) && <Row k="Depósito a pagar en el salón" v={eur(deposit)} accent />}
        </div>
      ...
      <div className="mt-6 flex gap-3">
        <Button type="button" variant="outline" onClick={downloadIcs} className="flex-1 rounded-full">
