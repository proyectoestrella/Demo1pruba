  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-x-6 gap-y-1 text-xs text-muted-foreground">
        <span>
          Servicio más pedido: <span className="font-medium text-foreground">{topService}</span>
        </span>
      </div>
                ...
                <Bar dataKey="bookings" radius={[6, 6, 0, 0]}>
                  {revData.slice(-14).map((d, i, arr) => (
                    <Cell
                      key={d.date}
                      fill="var(--color-primary)"
                      fillOpacity={i === arr.length - 1 ? 1 : 0.45}
                    />
                  ))}
                </Bar>
              </BarChart>
      ...
      <div data-tour="today-list" className="min-w-0 rounded-xl border border-border/60 bg-card">
        ...
        {todayList.length === 0 ? (
          <EmptyState
            ...
            description="Cuando reserves o crees una cita, aparecerá aquí."
          />
        ) : (
